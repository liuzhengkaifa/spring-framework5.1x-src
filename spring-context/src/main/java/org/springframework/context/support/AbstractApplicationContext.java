...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	static {
		// Eagerly load the ContextClosedEvent class to avoid weird classloader issues（主动加载ContextClosedEvent类以避免奇怪的类加载器问题）
	...
	/** Flag that indicates whether this context is currently active. （标志，指示此上下文当前是否处于活动状态。）*/
	...
	public void refresh() throws BeansException, IllegalStateException {
		//用于“刷新”和“销毁”的同步监视器，说白了就是一个锁-lz
		synchronized (this.startupShutdownMonitor) {
			// Prepare this context for refreshing.
			prepareRefresh();

			//获得一个BeanFactory,该方法会解析所有的spring配置文件（通常我们放于resources目录下，将所有的spring配置文件中的bean定义封装成BeanDefinition,加载到BeanFactory中，
			...
			// bean：environment、systemProperties 和 systemEnvironment，注册 2 个 bean 后置处理器：ApplicationContextAwareProcessor 和 ApplicationListenerDetector。
			...
			try {
				...
				// 主要用来在常规的 BeanFactoryPostProcessor 激活之前注册一些 bean 定义。特别是，你可以通过 BeanDefinitionRegistryPostProcessor 来注册一些常规的 BeanFactoryPostProcessor，
				...
				// 实例化所有剩余的非懒加载单例 bean。除了一些内部的 bean、实现了 BeanFactoryPostProcessor 接口的 bean、
				// 实现了 BeanPostProcessor 接口的 bean，其他的非懒加载单例 bean 都会在这个方法中被实例化，并且 BeanPostProcessor 的触发也是在这个方法中。
	...
	/**
	 * 更新方法的前置工作，记录了开始时间，上下文激活和关闭的状态，和一些属性源的初始化
